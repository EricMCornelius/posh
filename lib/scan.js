...
function Context(registry, file) {
  ...
  ctx.register = function(node) {
    node.env = ctx.env;
    node.base = node.base || node.env.base;
    registry.register(clone(node));
  }
  ctx.include = function(path) {
    console.log('Processing'.cyan, path.yellow);
    var contents = fs.readFileSync(path).toString();
    vm.runInNewContext(contents, new Context(registry, path));
  }
  return ctx;
};
...
function scan(registry, root, pattern) {
  ...
  async.waterfall([
    ...
    function(files, cb) { 
      var files = files.filter(function(file) { return path.basename(file) !== default_env_name; })
                       .map(async.apply(path.resolve, root).only(1));
      cb(null, files);
    },
