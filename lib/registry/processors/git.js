...
function register(node, registry, cb) {
  ...
  if (exists(config)) {
    ...
    config.branch = config.branch || config.tag || 'master';
...
function generate(graph, node, cb) {
  ...
  fs.stat(path.join(config.target, '.git'), function(err, info) {
    if (err) {
      var create_dir_cmd = create_directory_command(config.target);
      create_dir_cmd.action = 'update';
      ...
      graph.generated.push({
        ...
        cmds: [exists(config.tag) ? create_git_checkout_command(config) : create_git_checkout_track_command(config)],
