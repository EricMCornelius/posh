...
function create_wget_command(config) {
  return {
    sources: [],
    cmd: 'wget ' + config.url + ' -O ' + config.wget_target,
    action: 'update'
  };
}
...
function create_untar_command(config) {
  return {
    ...
    cmd: 'tar -xvf ' + config.wget_target + ' --keep-newer-files -C ' + config.targetdir,
    action: 'update'
  };
}
...
function create_unzip_command(config) {
  return {
    sources: [config.wget_target],
    cmd: 'unzip -u ' + config.wget_target + ' -d ' + config.targetdir,
    action: 'update'
  };
}
