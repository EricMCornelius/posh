...
function register(node, registry, cb) {
  ...
  var valid_types = ['application', 'shared_lib', 'static_lib', 'header_only', 'external', 'test'];
  ...
  node.incdir = path.resolve(node.env.base, node.incdir || node.env.incdir || 'include');
  node.srcdir = path.resolve(node.env.base, node.srcdir || node.env.srcdir || 'src');
  node.objdir = path.resolve(node.env.base, node.objdir || node.env.objdir || 'obj');
  node.libdir = path.resolve(node.env.base, node.libdir || node.env.libdir || 'lib');
  node.bindir = path.resolve(node.env.base, node.bindir || node.env.bindir || 'bin');
  node.installdir = path.resolve(node.env.base, node.installdir || node.env.installdir || 'install');
...
function parse_dependencies(compile_cmd, cb) {
  ...
  console.log('Generating C++ dependencies:');
  console.log(cmd);
...
function create_link_command(depinfo, node, compile_cmds) {
  ...
  var cmd = (node.type === 'static_lib') ?
    ...
    [node.linker].concat(node.linker_flags).concat(libdirs).concat(libs).concat(rpaths).concat(sources).concat(['-o', node.target]).join(' ');
