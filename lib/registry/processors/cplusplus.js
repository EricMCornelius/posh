...
const resolve = (resolver, path) => resolver(path);
...
function make_static_link_cmd(dirs, libs) {
  var parts = [];
  if (dirs) {
    const libpaths = [].concat(dirs).map(dir => `-L${dir}`).join(' ');
    parts.push(libpaths);
  }
  if (libs) {
    const is_linux = require('os').platform() === 'linux';
    const dynamic = is_linux ? '-Bdynamic' : '-dynamic';
    libs = [].concat(libs).map(lib => `-Wl,-static -l${lib} -Wl,${dynamic}`).join(' ');
    parts.push(libs);
  }
  return parts.join(' ');
}
  ...
  dgraph.recursive_visit(g, node, function(node) {
    ...
    switch (node.type) {
      ...
      case 'external':
        ...
        if (node.static) {
          linkcmds.push([prelink_flags, make_static_link_cmd(node.libdirs, node.libs), postlink_flags].join(' ').trim());
        }
        else {
          rpaths = rpaths.concat(node.rpaths);
          linkcmds.push([prelink_flags, make_dynamic_link_cmd(node.libdirs, node.libs), postlink_flags].join(' ').trim());
        }
        break;
      case 'header_only':
        includedirs = includedirs.concat(node.includedirs);
