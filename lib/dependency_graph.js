...
function DependencyGraphType(nodes, ignoreMissingDeps) {
  var self = this;

  self.ignore_missing_deps = ignoreMissingDeps || false;
...
function execute(g, args, cb) {
  ...
  error = args.error || function(err, node) { console.error(err.message); };
  cb = cb || function(err) {};
...
function write_graph(g) {
  var preamble = fs.readFileSync(path.resolve(__dirname, 'preamble.js')).toString();
  fs.writeFileSync('graph.gen', preamble + 'var graph = ' + serialize(g) + ';\nmodule.exports.graph = graph;\n\n');
};

var exports = [{DependencyGraph: DependencyGraphType}, recursive_visit, transitive_reduction, execute, write_graph];
