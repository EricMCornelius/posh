...
var public = require('./utils').public;
...
function DependencyGraphType(nodes, onMissingDependency) {
  ...
  self.on_missing_dependency = onMissingDependency || function(dep) { console.error('Missing dependency:', dep.red); };
...
function generate_node_map(g) {
  ...
  g.nodes = g.nodes.filter(function(node) {
    ...
    if (node.id in g.node_map) {
      console.log('[Warning]'.red + ' Duplicate node id: '.cyan + node.id.yellow);
      return false;
    }
...
function generate_target_map(g) {
  g.nodes.forEach(function(node) {
    ...
    if (target_path in g.target_map)
      throw new error.DuplicateTargetError({message: 'Duplicate target', target: target_path});
...
function dag_assert(g) {
  var check_cycle = function(id, colors, trace) {
    ...
    if (colors[id] === 'forward') {
      ...
      var message = front;
      var cycle = [front];
      var next = trace.shift();
      while(next) {
        cycle.push(next);
        message += ' -> ' + next;
        if (next === front)
          throw new error.CycleError({message: message, cycle: cycle});
        next = trace.shift();
      }
    }
...
function generational_sort(g) {
  ...
  function process_generation(arr) {
    var output = [];
    while (arr.length > 0) {
      var next = arr.shift();
...
function transitive_reduction(g) {
  ...
  var dfs = function(vertex0, child0) {
  ...
  Object.keys(discarded).forEach(function(key) {
...
function execute(g, args, cb) {
  ...
  var begin = args.begin || function() {};
  var end = args.end || function() {};
  var error = args.error || function(err, node) {
    console.log('Error processing node: '.red, node.id);
    throw new Error(err);
  };
  cb = cb || function() {};
  ...
  var process_next = function process_next() {
    var next = active.shift();
    if (exists(next)) {
      begin(next);

      var post_exec = function post_exec(err) {
        if (err)
          error(err, next);
        else
          end(next);
...
function write_graph(g, file) {
  ...
  var str = preamble + 'var graph = ' + serialize(g) + ';\nmodule.exports.graph = graph;\n\n';
  str = str.replace(/\\/g, '\\\\');
