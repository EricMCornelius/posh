...
function build(graph, node, cmd, cb) {
  ...
  if (!exists(cmd.target)) {
    ...
    var cmd = args.shift();
    var proc = spawn(cmd, args, {stdio: 'inherit', cwd: cmd.cwd || process.cwd()});
  ...
  async.map(cmd.sources, fingerprint, function(err, results) {
    ...
    console.log(cmd.cmd);
    exec(cmd.cmd, {cwd: cmd.cwd || process.cwd()}, cb);
  });
}
...
function generic(name, cb) {
  ...
  execute(graph, {action: name, begin: function(node) { console.log('Executing', name, 'for node:' + node.id); }}, function() { console.log('Done'); });
}
*/
