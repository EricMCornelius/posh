...
function build(graph, node, cmd, cb) {
  ...
  if (!exists(cmd.target)) {
    var current = cmd;
    current.sources = current.sources.map(async.apply(path.resolve, cmd.cwd || process.cwd()));
    async.map(current.sources, fingerprint, function(err, results) {
      ...
      var cwd = current.cwd || process.cwd();
  ...
  async.map(cmd.sources, fingerprint, function(err, results) {
    ...
    colored_exec(cmd.cmd, {cwd: cmd.cwd || process.cwd()}, cb);
  });
}
